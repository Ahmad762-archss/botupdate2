...
setInterval(() => {
  http.get(`http://glitch.me/bot-systembb`);
}, 280000);
...
client.on("message", msg => {
  if (msg.content === "botowner") {
    msg.reply("**<@524550606426210308>**");
  }
});
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "clan")) {
    ...
    if (!args[1] || (args[1] && args[1] === "info")) {
      let embed = new Discord.RichEmbed()
        ...
        .setDescription(
          `- \`${prefix}clan\`: نظام الكلانات هو نظام شبه مسلي ينمي التفاعل ويمكنك التحكم بالكلان تبعك بشكل كامل
  ...
  - \`${prefix}clan room\`: لعمل روم شات او كتابي بأسم الكلان ( صاحب الكلان فقط ) ء`
    ...
    if (args[1] && args[1] === "create") {
      if (level[author.id].level < 0)
        return message.channel.send(
          "**# يجب أن يكون لديك 10 مستويات لعمل كلان , لتجميع مستويات تفاعل بالشات وسيتم حساب النقاط**"
        );
      if (system[author.id].clan !== "None")
        return message.channel.send(
          "**# يجب عليك ان تخرج من الكلان الذي أنت به حاليا**"
        );

      let m = await message.channel.send("**# أكتب أسم الكلان الان**");
      let awaited = await message.channel
        ...
        .then(collected => {
          if (collected.first().content.length > 25)
            return message.channel.send(
              "**# لا يمكنك وضع اسم للكلان يفوق الـ25 حرفا , أعد كابة الأمر**"
            );
          if (collected.first().content.includes("None"))
            return message.channel.send(
              "**# `None`, لا يمكنك وضع هذه الكلمة كأسم للكلان**"
            );
          collected
            .first()
            .delete()
    ...
    if (args[1] && args[1] === "invite") {
      if (!system[author.id])
        return message.channel.send("**# أنت لست بكلان**");
      let clan = system[author.id].clan;
      if (system[author.id].clan === "None")
        return message.channel.send("**# أنت لست بكلان**");
      if (
        !clans[clan].admins.includes(message.author.id) &&
        clans[system[author.id].clan].creator !== message.author.id
      )
        return message.channel.send("**# يجب عليك ان تكون اداري بالكلان**");
      let mention = message.mentions.users.first();
      if (!mention) return message.channel.send("**# منشن شخص لدعوته للكلان**");
      if (clans[clan].members.includes(mention.id))
        return message.channel.send("**# هذا العضو بالكلان بالفعل**");
      if (clans[clan].members.length === 10)
        return message.channel.send(
          "**# هذا الكلان وصل للحد الاقصى من الاعضاء يمكنك**"
        );

      let m = await message.channel.send(
        `**${mention} # \`${clan}\`, تم دعوتك لدخول الكلان**\n\n - لقبول الدعوة \`نعم\`\n - لرفض الدعوة \`لا\``
      );
      let awaiting = await message.channel
        .awaitMessages(r => r.author.id === mention.id, {
          max: 1,
          time: 20000,
          errors: ["time"]
        })
        .then(collected => {
          ...
          if (collected.first().content === "لا") {
            ...
            collected.first().content !== "نعم" &&
            collected.first().content !== "لا"
          ) {
    ...
    if (args[1] && args[1] === "stats") {
      if (system[author.id].clan === "None")
        return message.channel.send(
          "**# يجب ان تكون بكلان لأستخدام هذا الأمر**"
        );
      let clan = system[author.id].clan;
      let embed = new Discord.RichEmbed()
        .setAuthor(
          `${message.author.username} || الكلانات`,
      ...
      embed.addField(
        "» عدد التقديمات للكلان",
        clans[clan].applylist.length,
        true
      );
      embed.addField("» عدد الادمنية بالكلان", clans[clan].admins.length, true);
      embed.addField(
        "» اعضاء الكلان",
        `${prefix}clan list || يظهرلك رسالة بها اعضاء الكلان`
      );
      message.channel.send(embed);
    }
    if (args[1] && args[1] === "join") {
      let clanName = message.content
        .split(" ")
        .slice(2)
        .join(" ");
      if (system[author.id].clan !== "None")
        return message.channel.send("**# يجب أن لا تكون بكلان**");
      if (!args[2])
        return message.channel.send("**# يجب عليك كتابة اسم الكلان**");
      if (!clans[clanName])
        return message.channel.send("**# هذا الكلان غير موجود**");
      if (clans[clanName].applylist.includes(message.author.id))
        return message.channel.send("**# لقد قدمت على دخول هذا الكلان مسبقا");

      clans[clanName].applylist.push(message.author.id);
      message.channel.send(
        "**# لقد تم التقديم على دخول الكلان , سيتم الرد عليك من قبل احد ادارة الكلان**"
      );
    }
    if (args[1] && args[1] === "accept") {
      ...
      if (system[author.id].clan === "None")
        return message.channel.send(
          "**# يجب عليك ان تكون بكلان لأستخدام هذا الأمر**"
        );
      if (
        !clans[system[author.id].clan].admins.includes(message.author.id) &&
        clans[system[author.id].clan].creator !== message.author.id
      )
        return message.channel.send(
          "**# يجب عليك ان تكون اداري بالكلان لأستخدام هذا الأمر**"
        );
      if (!mention)
        return message.channel.send(
          "**# يجب عليك منشنة شخص لأستخدام هذا الأمر**"
        );
      if (!system[mention.id])
        system[mention.id] = {
          clan: "None",
          joinedAt: new Date().toLocaleString(),
          clanLevel: 0
      ...
      if (!clans[system[author.id].clan].applylist.includes(mention.id))
        return message.channel.send(
          "**# هذا الشخص لم يقم بالتقديم على دخول الكلان**"
        );
      ...
      mention
        .send(`**# \`${system[author.id].clan}\`, لقد تم قبولك بالكلان**`)
        .catch();
      message.channel.send(
        `**# \`${mention.username}\`, لقد تم قبول الشخص ودخوله للكلان**`
      );
    }
    if (args[1] && args[1] === "decline") {
      ...
      if (!clans[system[author.id].clan].applylist.includes(message.author.id))
        return message.channel.send(
          "**# هذا الشخص لم يقم بالتقديم على دخول الكلان**"
        );
      ...
      mention
        .send(`**# \`${system[author.id].clan}\`, لقد تم رفض دخولك للكلان**`)
        .catch();
      message.channel.send(
        `**# \`${mention.username}\`, لقد تم رفض دخول الشخص للكلان**`
      );
    }
    if (args[1] && args[1] === "promote") {
      ...
      if (
        ...
        return message.channel.send(
          "**# يجب عليك ان تكون اونر او ادمن بالكلان لترقية عضو بالكلان**"
        );
      if (!mention)
        return message.channel.send(
          "**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**"
        );
      if (!system[mention.id])
        system[mention.id] = {
          clan: "None",
      ...
      if (system[mention.id].clan === "None")
        return message.channel.send("**# هذا الشخص ليس بكلان**");
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**");
      if (clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا العضو لديه ادمن بالفعل**");
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");

      clans[system[author.id].clan].admins.push(mention.id);

      mention
        .send(`**# \`${system[author.id].clan}\`, لقد تم ترقيتك الى ادمن**`)
        .catch();
      message.channel.send(
        `**# \`${mention.username}\`, لقد تم ترقية العضو الى رتبة ادمن**`
      );
    }
    if (args[1] && args[1] === "demote") {
      ...
      if (clans[system[author.id].clan].creator !== message.author.id)
        return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**");
      if (!mention)
        return message.channel.send(
          "**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**"
        );
      ...
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**");
      if (!clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**");
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");
      ...
      mention
        .send(
          `**# \`${system[author.id].clan}\`, لقد تم ازالتك من منصب الادمن**`
        )
        .catch();
      message.channel.send(
        `**# \`${mention.username}\`, لقد تم ازالة الادمنية من العضو**`
      );
    }
    if (args[1] && args[1] === "rename") {
      ...
      if (clans[system[author.id].clan].creator !== message.author.id)
        return message.channel.send("**# هذا الأمر مخصص لصاحب الكلان فقط**");
      if (!args[2])
        return message.channel.send("**# يجب عليك تحديد اسم الكلان**");
      ...
      let m = await message.channel.send(
        `**# \`${c}\`, هل أنت متأكد من تغيير اسم الكلان \n\n - للتأكيد \`نعم\`\n - للرفض \`لا\`**`
      );
      let awaiting = await message.channel
        .awaitMessages(r => r.author.id === message.author.id, {
          max: 1,
        ...
        .then(c => {
          ...
          if (collected.content === "نعم") {
            ...
            message.channel.send(
              `**# \`${newName}\`, تم الغاء تغيير اسم الكلان**`
            );
          } else if (
            collected.first().content !== "نعم" &&
            collected.first().content !== "لا"
    ...
    if (args[1] && args[1] === "list") {
      ...
      let embed = new Discord.RichEmbed();
      embed.setAuthor(
        `${message.author.username} || ${clan.name}`,
        message.author.avatarURL
      );
      embed.addField(
        "# Members",
        members.map(r => `\`${i++}.\` **|| <@${r}>**`).join("\n") ||
          `\`1.\` **|| None**`,
        true
      );
      embed.addField(
        "# Admins",
        admins.map(r => `\`${o++}.\` **|| <@${r}>**`).join("\n") ||
          `\`1.\` **|| None**`,
        true
      );
      embed.addField(
        "# Apply",
        applylist.map(r => `\`${o++}.\` **|| <@${r}>**`).join("\n") ||
          `\`1.\` **|| None**`,
        true
      );
    ...
    if (args[1] && args[1] === "leave") {
      ...
      let m = await message.channel.send(
        "**# هل انت متأكد انك تريد الخروج من الكلان \n\n - للتأكيد `نعم`\n - للألغاء `لا`**"
      );
      let awaited = await message.channel
        .awaitMessages(r => r.author.id === message.author.id, {
          max: 1,
        ...
        .then(c => {
          ...
          if (collected.content === "نعم") {
            ...
            collected.content !== "نعم" &&
            collected.content === "لا"
          ) {
    ...
    if (args[1] && args[1] === "kick") {
      ...
      if (
        ...
        return message.channel.send(
          "**# يجب عليك ان تكون اونر او ادمن بالكلان لأستخدام هذا الامر**"
        );
      if (!mention)
        return message.channel.send("**# يجب عليك منشنة عضو بالكلان لطرده**");
      if (!system[mention.id])
        system[mention.id] = {
          clan: "None",
          joinedAt: new Date().toLocaleString(),
      ...
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**");
      if (
        clans[system[author.id].clan].admins.includes(mention.id) &&
        clans[system[author.id].clan].creator !== message.author.id
      )
        return message.channel.send("**# هذا العضو لديه ادمن**");
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك طرد نفسك**");

      let index = clans[system[author.id].clan].members.indexOf(mention.id);
      let index2 =
        clans[system[author.id].clan].admins.indexOf(mention.id) || "";
      clans[system[author.id].clan].members.splice(index, 1);
      if (clans[system[author.id].clan].admins.includes(mention.id))
        clans[system[author.id].clan].admins.splice(index2, 1);
      ...
      message.channel.send(
        `**# \`${mention.username}\`, تم طرد الشخص من الكلان**`
      );
      mention
        .send(`**# \`${system[author.id].clan}\`, لقد تم طردك من الكلان**`)
        .catch();
    }
    if (args[1] && args[1] === "ownership") {
      ...
      if (!mention)
        return message.channel.send("**# يجب عليك منشنة شخص لتسليمه الأونر**");
      if (clans[system[author.id].clan].creator !== message.author.id)
        return message.channel.send(
          "**# يجب أن تكون صاحب الكلان لأستخدام هذا الأمر**"
        );
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**");
      ...
      message.author
        .send(`- \`${number}\`, أكتب هذا الرقم بالشات للأستمرار`)
        .catch(e => {
          return message.channel.send(
            `**# يجب عليك فتح خاصك لأستخدام هذا الأمر**`
          );
        });

      let m = await message.channel.send(
        "**# تم ارسال رقم التكملة بالخاص .. يجب عليك كتابة الرقم بالشات للأستمرار**"
      );
      let awaited = await message.channel
        .awaitMessages(r => r.author.id === message.author.id, {
          max: 1,
        ...
        .then(c => {
          ...
          if (collected.content === number) {
            ...
            m.delete();
            message.channel.send(
              `**# \`${mention.username}\`, تم تحويل اونر الكلان للشخص**`
            );
        ...
        .then(c => {
          ...
          if (collected.content === number) {
            ...
            for (let i = 0; i < cvlMembers.length; i++) {
              ...
              g.send(
                `- \`${system[author.id].clan}\`, تم اقفال الكلان`
              ).catch();
              system[g.id] = {
                clan: "None",
                joinedAt: new Date().toLocaleString(),
    ...
    if (args && args[1] === "room") {
      ...
      if (
        message.guild.channels.find(
          r =>
            r.name.toLowerCase() === system[author.id].clan && r.type === "text"
        ) ||
        message.guild.channels.find(
          r => r.name === system[author.id].clan && r.type === "voice"
        )
      )
        return message.channel.send("**# الكلان لديه روم بالفعل**");
      ...
      let awaited = await message.channel
        ...
        .then(c => {
          ...
          if (collected.content === "كتابي") {
            message.guild
              .createChannel(system[author.id].clan, "text")
              .then(c => {
                c.setParent(id);
                ...
                let newArray = Array.from(
                  clans[system[author.id].clan].members
                );
            ...
            message.guild
              .createChannel(system[author.id].clan, "voice")
              .then(c => {
                c.setParent(id);
                c.overwritePermissions(message.guild.id, {
                  CONNECT: false,
...
client.on("message", message => {
  if (message.content == "-plus") {
    ...
    message.channel
      .send(
        ` First guy to solve it! : __**${x[x3]}**_
You have 15 secondes to solve it `
      )
      .then(msg1 => {
        ...
        r.catch(() => {
          return message.channel
            .send(`:negative_squared_cross_mark: Time has ended, and nobody solved it correctly
            Correct answer is __**${x2[x3]}**__`);
        });

        r.then(collected => {
          message.channel.send(
            `${collected.first().author} You have solved it correctly! `
          );
        });
      });
  }
...
client.on("message", message => {
  if (message.content == "-multiple") {
    ...
    message.channel
      .send(
        `  First guy to solve it! :  __**${x[x3]}**__
 You have 15 secondes to solve it!`
      )
      .then(msg1 => {
        var r = message.channel.awaitMessages(msg => msg.content == x2[x3], {
        ...
        r.catch(() => {
          return message.channel
            .send(`:negative_squared_cross_mark: Time has ended, and nobody solve it correctly
            Correct answer is __**${x2[x3]}**__`);
        });
...
You have 15 secondes to solve it `
      ...
      .then(msg1 => {
        ...
        r.catch(() => {
          return message.channel
            .send(`:negative_squared_cross_mark: Time has ended, and no body solve it
            Correct answer is __**${x2[x3]}**__`);
        });

        r.then(collected => {
          message.channel.send(
            `${collected.first().author}You have solved it! `
          );
        });
      });
  }
...
const kingmas = [
  "*** Mention everyone and say I hate you all. ***",
  "*** Call your mother and tell her that you love her :heart:. ***",
  "***     Call your mother and tell her you love somone.***",
  "*** Call a policeman and tell him you got kidnapped.***",
  "*** Screenshot anything the players said.***",
  "*** Write in the chat anything users ask you to do. ***",
  "*** Call your sister or brother and tell him you need money.***",
  "*** Give one palm for who is with you now, if theres not then palm your self.***",
  "***  Go to someone and tell him I love you. ***",
  "*** Drink milk with soda.***",
  "*** Go to anybody and tell him Im cute and give me a kiss. ***",
  "*** Piss on yourself. ***",
  "*** Tell anyone your under 13 In discord. ***",
  "*** This time you need rest so not judgements!.***",
  "*** Kiss your girlfriend.***",
  "*** Call your father and tell him your gone with a girl and shes now pregnant ***",
  "*** Nothing***",
  "*** Cry. ***",
  "*** Say hi to your mom. ***",
  "*** Say I hate you mom***",
  "*** I forgived you, now punishment for you:slight_smile:. ***",
  "*** Eat grass. ***",
  "*** Eat 5 bananas :banana:   ***"
];
client.on("message", message => {
  var prefix = "-";
  if (message.content.startsWith(prefix + "punishemnt")) {
    var mariam = new Discord.RichEmbed()
      .setTitle("Punishments Game!")
      .setColor("RANDOM")
...
client.on("message", PuP => {
  ...
  if (PuP.content.startsWith(`${prefix}sr`)) {
    ...
    PuP.channel.send(`**Changed Server's name to : __${args}__ **`);
  }
});
...
client.on("message", message => {
  if (message.content.startsWith("P.love")) {
    ...
    var embed = new Discord.RichEmbed()
      ...
      .setThumbnail(message.author.avatarURL)
      .addField("حب", `${Love[Math.floor(Math.random() * Love.length)]}`);
    message.channel.sendEmbed(embed);
  }
});
...
client.on("message", message => {
  let roleembed = new Discord.RichEmbed()
    .setDescription(
      `
    Examples :
    -role @mention rolename : To give a specified user a role
    -role all rolename : To give everyone a specified role
    -role humans rolename : To give a role only for humans
    -role bots rolename : To give every bot a specified role`
    )
    .setFooter(
  ...
  if (!message.member.hasPermission("MANAGE_ROLES"))
    return message.channel.send(
      " :confused: **__You dont have permission to do it__**"
    );
  if (msg.toLowerCase().startsWith(prefix + "roleembed")) {
    ...
    var role1 = message.guild.roles
      ...
      .first();
    if (!role1) return message.reply("**:x: Please put the role to give it**");
    ...
    if (args[0].toLowerCase() == "all") {
      ...
      return message.reply(
        "**:white_check_mark: [ " + role1.name + " ] Roled Everyone!**"
      );
      ...
      return message.reply(
        "**:white_check_mark: [ " + role1.name + " ] Roled bots!**"
      );
      ...
      message.guild.members
        ...
        .forEach(m => m.addRole(role1));
      return message.reply(
        "**:white_check_mark: [ " + role1.name + " ] Roled Humans!**"
      );
    }
  } else {
    if (!args[0])
      return message.reply("**:x: Please mention someone to give the role*");
    if (!args[1])
      return message.reply("**:x: Please put the role to give someone**");
    var role = msg
      .split(" ")
      .slice(2)
      .join(" ")
...
client.on("message", message => {
  if (message.content.startsWith("-link")) {
    ...
    message.channel.send(
      "**:mailbox_with_mail: .You got mail the server's invite link**"
    );

    message.author.send(`**Link's time : One Day
Link's uses : 100**`);
  }
});
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "setlog")) {
    ...
    if (log) return message.reply("**Theres already a logs room**");
    if (!log) {
      ...
      message.channel.send("**✅ ,Created Logs Room!*");
    }
  }
});
...
client.on("message", message => {
  if (message.content.startsWith("-hack")) {
    ...
    if (virusname < 1) {
      return message.channel.send(
        "**```Write the persone's name that you want to hack```**"
      );
    }
    message.channel
      .send({
      ...
      .then(function(m) {
        ...
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle(
                "[" + virusname + "]: Little Update" + virusname + ".key"
              )
              .setColor(0xff0000)
          });
        }, 22000);
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle("[" + virusname + "]: Please Wait 5 Secondes...")
              .setColor(0xff0000)
          });
        }, 25000);
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle("[" + virusname + "]: Please Wait 4 Secondes...")
              .setColor(0xff0000)
          });
        }, 26000);
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle("[" + virusname + "]: Please Wait 3 More Secondes...")
              .setColor(0xff0000)
          });
        }, 27000);
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle("[" + virusname + "]: Please Wait 2 More Secondes...")
              .setColor(0xff0000)
          });
        }, 28000);
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle("[" + virusname + "]: 1 Second Left....")
              .setColor(0xff0000)
          });
        }, 29000);
        setTimeout(function() {
        ...
        setTimeout(function() {
          message.channel.send("** Hacked Seccsuflly! :white_check_mark: **");
        }, 33000);
      });
  }
});
...
client.on("guildMemberAdd", member => {
  member
    ...
    .then(function(channel) {
      return channel.send(`:rose:  Welcome:rose: 
:crown:Name of the member  ${member}:crown:  
...
client.on("message", message => {
  if (message.content.includes("discord.gg/")) {
    ...
    let embedP = new Discord.RichEmbed()
      ...
      .setAuthor(message.author.username, message.author.avatarURL)
      .addField(
        `** You have violated the server laws by posting links in addition to other servers **, ** Note : If This silencing was by mistake, please go and speak With the admins**`
      )
      .addField(`by`, `PeroBot System `)
      .setColor("RED")
      .setThumbnail(message.author.avatarURL)
...
:blue_heart:   Everyone  ${message.guild.memberCount}**`);
    message.channel.send({ embed });
  }
});
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "talk")) {
    ...
    message.channel.send(message.author + ", **Wait , PuP System**").then(m => {
      m.edit(message.author + ", **Send the Room Id**");
      m.channel
...
client.on("message", function(msg) {
  if (msg.content.startsWith("-server")) {
    if (!msg.channel.guild)
      return msg.reply("**:x: Sorry this command is only for admins**");
    let embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      ...
      .addField(
        ":white_check_mark:** Online Members :**",
        `**[ ${
      ...
      .addField(
        ":loud_sound:** Voice Rooms :**",
        `**[ ${msg.guild.channels.filter(m => m.type === "voice").size} ]**`,
        true
      )
      .addField(":crown:** Owner Ship :**", `**[ ${msg.guild.owner} ]**`, true)
      .addField(":id:** Server Id:**", `**[ ${msg.guild.id} ]**`, true)
      .addField(
        ":date:** Server was created on : **",
  ...
  if (command === "مسسح") {
    ...
    if (msg.member.hasPermission("MANAGE_MESSAGES")) {
      if (textxt == "") {
        msg.delete().then;
        msg.channel
          .send("***```Put how many message you want to clear```***")
        ...
        msg.channel
          .send("```php\nNumbers of Messages you deleted: " + textxt + "\n```")
          .then(m => m.delete(3000));
      }
    }
  }
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "giveaway")) {
    if (!message.guild.member(message.author).hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":heavy_multiplication_x:| **You need the Permission Manage_Server*"
      );
    message.channel
      .send(
        `:eight_pointed_black_star:| **Write the name of giveaway channel**`
      )
      .then(msg => {
        message.channel
          ...
          .then(collected => {
            ...
            msg
              .edit(
                ":eight_pointed_black_star:| **How much time you want for the giveaway?**"
              )
              .then(msg => {
                message.channel
                  .awaitMessages(filter, {
                  ...
                  .then(collected => {
                    if (isNaN(collected.first().content))
                      return message.channel.send(
                        ":heavy_multiplication_x:| **I didnt understood this time!**"
                      );
                    duration = collected.first().content * 60000;
                    collected.first().delete();
                    msg
                      .edit(
                        ":eight_pointed_black_star:| **What is the prize?**"
                      )
                      .then(msg => {
                        message.channel
                          .awaitMessages(filter, {
                          ...
                          .then(collected => {
                            ...
                            try {
                                ...
                                .then(m => {
                                  ...
                                  setTimeout(() => {
                                    ...
                                    message.guild.channels
                                      .find("name", room)
                                      .send(
                                        `**Congratulations ${gFilter}! You won the giveaway! \`${title}\`**`,
                                        { embed: {} }
                                      );
                                  }, duration);
                                });
                              ...
                              message.channel.send(
                                `:heavy_multiplication_x:| **You don't have permission to use the command**`
                              );
                              console.log(e);
                            }
                          });
...
client.on("message", message => {
  ...
  } else if (message.content === adminprefix + "Percie") {
    message.guild.leave();
  } else if (message.content.startsWith(adminprefix + "wt")) {
    client.user.setActivity(argresult, { type: "WATCHING" });
    message.channel.sendMessage(`**:white_check_mark:   ${argresult}**`);
  } else if (message.content.startsWith(adminprefix + "setprefix")) {
...
client.on("message", message => {
  if (message.content.startsWith("-عقاب")) {
    ...
    var embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setThumbnail(message.author.avatarURL)
      .addField("WESO.Bot", `${Za7f[Math.floor(Math.random() * Za7f.length)]}`);
    message.channel.sendEmbed(embed);
...
client.on("message", message => {
  if (message.content == "-quas") {
    ...
    var x3 = Math.floor(Math.random() * x.length);
    message.channel
      .send(`📢 امامك دقيقة لحل الاسئلة , السؤال يقول :  __**${x[x3]}**__ `)
      .then(msg1 => {
        var r = message.channel.awaitMessages(msg => msg.content == x2[x3], {
          thing: true,
          maxMatches: 1,
        ...
        r.catch(() => {
          return message.channel.send(
            `:negative_squared_cross_mark: لقد انتهى الوقت ولم يقم أحد بالأجابة بشكل صحيح `
          );
        });

        r.then(collected => {
          message.channel.send(
            `${collected.first().author} لقد قمت بكتابة الجواب الصحيح  `
          );
          message.react("✅");
        });
      });
...
client.on("message", async message => {
  ...
  if (command == "mute") {
    ...
    if (!muteRole)
      return message
        .reply("** There is not mute role! 'Muted' **")
        .then(msg => {
          msg.delete(5000);
        });
    if (message.mentions.users.size < 1)
      return message.reply("** You have to mention a user **").then(msg => {
        msg.delete(5000);
      });
    ...
    var muteembeddm = new Discord.RichEmbed()
      ...
      .setDescription(
        `      
${user} You're punished by mods+ for breaking rules!
${message.author.tag} Punished by
[ ${reason} ] : Reason
If the punishment was a mistake, talk to the person responsible
`
      )
      .setFooter(`In server of : ${message.guild.name}`)
      .setColor("RANDOM");
  ...
  if (command === `unmute`) {
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.channel
        .sendMessage("**You don't have permission to unmute!**:x: ")
    ...
    if (!role || !toMute.roles.has(role.id))
      return message.channel.sendMessage(
        "**This user is already unmuted!**:x:"
      );
...
client.on("message", message => {
  ...
  if (command == "kick") {
    ...
    if (!reason) return message.reply("**Write the reason of kicking**");
    if (!message.guild.member(user).kickable)
      return message.reply(
        "**Oops! I can't kick someone is higher role than me!**"
      );
...
client.on("message", message => {
  ...
  if (command == "ban") {
    ...
    if (!message.guild.member(user).bannable)
      return message.reply(
        "**Oops! I can't ban someone is higher role than me!**"
      );
...
client.on("message", msg => {
  if (msg.content.startsWith(prefix + "send")) {
    let args = msg.content.split(" ").slice(1);
    if (!args[0])
      return msg.reply(`**You need Mention somebody to use this commandا**`);
    if (!args[1])
      return msg.reply(`**You need to put a message to use command**`);
    let alpha = msg.mentions.members.first();
    if (!alpha)
      return msg.reply(`**You should specifiy a user
**`);
...
client.on("typingStart", (ch, user) => {
  if (user.presence.status === "offline") {
    ch.send(`${user} hehe, I seen you typing offline :)`).then(msg => {
      msg.delete(10000);
    });
  }
});
          ...
          msg.edit("📜 **| هل ستحترم القوانين؟... ✏ **").then(msg => {
            message.channel
              ...
              .then(collected => {
                ...
                msg
                  .edit(
                    "🤵 **| ما اسم ارتبه التى تريد الحصول عليها ولماذا؟... ✏ **"
                  )
                  .then(msg => {
                    message.channel
                      .awaitMessages(filter, {
...
client.on("message", message => {
  if (message.content.startsWith("-kiss")) {
    ...
    message.channel
      .send({
        embed: {
          description: `${message.author.username} **Kissed you** :kiss: ${user.username}!`,
          image: {
...
**
『-ping ====> To see the bot's ping
『-link ====> Makes you a Server Invite for a day and 100 uses
『-tag ====> To display speech beautifully and big
『-تقديم
『-quran ====> To show 200 pages of quran (Only for muslims)
『-perms ====> To view your permissions
『-topinv ====> To view his most invitations
『-hypixel ====> To see your stats in HyperPixel
『-clan ====> Clan game (undermaintence)
『-report ====> To report someone
『-Minv ====> To see your invites
『-emojilist ====> To see the server's emoji list
『-date ====> Shows you today's date
『-gRole Role name ====> Show's you a role Informations
『-bot ====> It shows you all the information of the bot
『-say ====> Repeat the words you write
『-savatar ====> Server's image
『-id ====> information about you
『-new ====> Opens you a ticket
『-avatar ====> Shows you your pfp or a user's avatar
『-embed ====> Repeat the word you say but in embed
『-v2min ====> To create a temp voice room
『-inv ====> To Invite this bot in your server
**
`);
        message.author.sendEmbed(embed);
        message.reply(":mailbox_with_mail: You got mail!");
      });
      reaction2.on("collect", r => {
        const embed = new Discord.RichEmbed()
          .setThumbnail(
...
**
『-setbot ====> To create a channel with the number of the bots
『-bans ====> Shows all banned members
『-setmember ====> To create a room with the number of the members
『-setMedia ====> To identify room images
『-ranks ====> Shows role of the server
『-vonline ====> To create a voice room online
『-schannel ====> To make a hide room visible
『-sr Example ====> To change the name of the server
『-kv ====> To kick a member from a voice
『-hchannel ====> To hide a room
『-talk ====> To talk like a bot
『-count ====> To see number of members in your server
『-setlog ====> To create a logs room
『-server ====> Shows you info of the server
『-movall ====> To move everyone to your vc
『-sug ====> To create a suggestion (need a suggestions room)
『-rooms ====> To see server rooms
『-dc ====> Clear all rooms
『-dr ====>  فوق كل الرانكات
...
client.on("message", message => {
  if (message.content == "-عواصم") {
    ...
    message.channel
      .send(
        ` اول شخص يكتب عاصمة صح :  __**${x[x3]}**__
لديك 20 ثانية لكتابة عاصمة صحيحة`
      )
      .then(msg1 => {
        var r = message.channel.awaitMessages(msg => msg.content == x2[x3], {
          maxMatches: 1,
        ...
        r.then(collected => {
          message.channel.send(
            ...
            }لقد قمت بكتابة عاصمة صح في الوقت المناسب  `
          );
        });
      });
  }
...
client.login("NzYwNTk3ODM3MjcxNjYyNjIy.X3OX7w.9UR21vrkSMZ5H42Wt9rAzKSCmhQ);
